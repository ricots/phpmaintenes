...
<body>
<form action="simpan_maintenes_software.php" name="postform" method="post">
...
<table width="880" border="1">
  <tr>
   ...
   <?php
    ...
    <td>
      <?php
$kondisi=array('kosong','test');
foreach($kondisi as $k)
echo "<input type='checkbox' name='kondisi[{$lihat['id_komputer']}]' value='{$k}|{$lihat['id_ruang']}|{$lihat['id_komputer']}|".date('Y-m-d').",'kondisi1[]'' />{$k}\n";
?></td>
    <td><?php echo $lihat['program_web'] ?></td>
    <td>
    <input type="checkbox" name="kondisi1[]" value="ada">ada <input type="checkbox" name="kondisi1[]" value="kosong">kosong
    </td>
    <td><?php echo $lihat['program_office'] ?></td>
    <td>
    <input type="checkbox" name="kondisi2[]" value="ada">ada <input type="checkbox" name="kondisi2[]" value="kosong">kosong
    </td>
  </tr>  
  <?php
}
